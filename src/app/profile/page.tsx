    ...
    <main className="mx-auto w-full max-w-6xl flex-1 px-4 py-8 sm:px-6 sm:py-10 lg:px-8">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3.5 sm:gap-4">
          <div className="flex h-14 w-14 sm:h-16 sm:w-16 items-center justify-center rounded-2xl bg-primary text-xl sm:text-2xl font-bold text-primary-foreground shadow-xs">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-muted-foreground">
              {profile
                ? profile.email
                : "Signed out — progress is saved in local browser storage."}
            </p>
          </div>
        </div>
        <Button variant="outline" size="sm" className="h-8.5 text-xs sm:text-sm" onClick={handleReset}>
          <RotateCcw className="mr-1.5 h-3.5 w-3.5 text-muted-foreground" /> Reset progress
        </Button>
      </div>

      <div className="mt-6 sm:mt-8 rounded-xl border bg-card p-4 sm:p-5 shadow-xs">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="min-w-0 flex-1">
            <h2 className="text-base sm:text-lg font-semibold">Backup &amp; sync your data</h2>
            <p className="mt-1 text-xs sm:text-sm text-muted-foreground leading-relaxed">
              Export your progress as a JSON file to keep it safe or move it to another
              machine — import it back anytime to restore or merge.
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2 shrink-0">
            <Button variant="outline" size="sm" className="h-8.5 text-xs sm:text-sm" onClick={handleExport}>
              <Download className="mr-1.5 h-3.5 w-3.5" /> Export data
            </Button>
            <Button variant="outline" size="sm" className="h-8.5 text-xs sm:text-sm" onClick={() => fileInputRef.current?.click()}>
      ...
      <div className="mt-6 sm:mt-8 grid gap-3 sm:gap-4 sm:grid-cols-3">
        <div className="rounded-xl border bg-card p-4 sm:p-5 shadow-xs">
          <div className="flex items-center gap-2 text-xs font-medium uppercase tracking-wider text-muted-foreground">
            <Target className="h-3.5 w-3.5 text-primary" /> Solved
          </div>
          <div className="mt-2 text-3xl sm:text-4xl font-extrabold tracking-tight tabular-nums">
            {solvedCount}
            <span className="text-base sm:text-lg font-normal text-muted-foreground"> / {total}</span>
          </div>
          <div className="mt-3 h-2 overflow-hidden rounded-full bg-muted">
            <div className="h-full rounded-full bg-green-500 transition-all duration-500" style={{ width: `${pct}%` }} />
          </div>
          <p className="mt-2 text-xs text-muted-foreground">{pct}% of the curated roadmap</p>
        </div>
        ...
        <div className="rounded-xl border bg-card p-4 sm:p-5 shadow-xs">
          ...
          <div className="mt-2 text-3xl sm:text-4xl font-extrabold tracking-tight tabular-nums">
            {streak}
            <span className="text-base sm:text-lg font-normal text-muted-foreground"> day{streak === 1 ? "" : "s"}</span>
          </div>
          <p className="mt-3 text-xs text-muted-foreground">
            {streak > 0 ? "Keep solving daily to maintain streak!" : "Solve a problem today to start a streak."}
          </p>
        </div>

        <div className="rounded-xl border bg-card p-4 sm:p-5 shadow-xs">
          <div className="flex items-center gap-2 text-xs font-medium uppercase tracking-wider text-muted-foreground">
            <CalendarDays className="h-3.5 w-3.5 text-primary" /> Attempted
          </div>
          <div className="mt-2 text-3xl sm:text-4xl font-extrabold tracking-tight tabular-nums">{attemptedCount}</div>
          <p className="mt-3 text-xs text-muted-foreground">Problems you have tested code against</p>
        </div>
      </div>

      <div className="mt-6 sm:mt-8 grid gap-4 sm:gap-6 lg:grid-cols-2">
        <section>
          <h2 className="text-base sm:text-lg font-semibold">Difficulty breakdown</h2>
          <div className="mt-3 space-y-2.5 rounded-xl border bg-card p-4 shadow-xs">
            {(["Easy", "Medium", "Hard"] as const).map((d) => {
              ...
              return (
                <div key={d} className="flex items-center gap-3 text-xs sm:text-sm">
        ...
        <section>
          <h2 className="text-base sm:text-lg font-semibold">Category progress</h2>
          <div className="mt-3 max-h-96 space-y-1.5 overflow-y-auto rounded-xl border bg-card p-4 shadow-xs">
            {CATEGORIES.map((cat) => {
              ...
              return (
                <Link
                  key={cat.id}
                  href={`/roadmap#${cat.id}`}
                  className="flex items-center gap-3 rounded-lg px-2 py-1.5 text-xs sm:text-sm transition-colors hover:bg-accent/60"
                >
                  ...
                  <div className="h-1.5 w-16 sm:w-20 overflow-hidden rounded-full bg-muted shrink-0">
                    <div className="h-full rounded-full bg-green-500 transition-all duration-500" style={{ width: `${w}%` }} />
                  </div>
                  <span className="w-10 text-right tabular-nums text-xs text-muted-foreground shrink-0">
      ...
      <section className="mt-8">
        ...
        {recent.length === 0 ? (
          <div className="mt-3 rounded-xl border bg-card p-8 text-center text-sm text-muted-foreground shadow-xs">
            Nothing yet — <Link href="/problems" className="font-medium text-foreground underline underline-offset-2 hover:text-primary">try a problem</Link>.
          </div>
        ) : (
          <div className="mt-3 overflow-hidden rounded-xl border bg-card shadow-xs">
            {recent.map((s) => (
              <Link
                key={s.id}
                href={`/problems/${s.slug}`}
                className="flex items-center gap-3 border-b px-4 py-3 text-xs sm:text-sm transition-colors last:border-b-0 hover:bg-accent/60"
              >
                <CheckCircle2
                  className={cn(
                    "h-4 w-4 shrink-0",
                ...
                {s.status === "Accepted" && (
                  <span className="hidden w-16 text-right tabular-nums text-xs text-muted-foreground sm:block">
                    {formatRuntime(s.runtimeMs)}
                  </span>
                )}
                <span className="w-24 sm:w-28 text-right text-xs tabular-nums text-muted-foreground shrink-0">
